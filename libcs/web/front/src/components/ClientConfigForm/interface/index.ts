...
export namespace ClientConfigBackend {
  export interface Config extends Options {
    Services?: Service[];
  }
...
export function transToBackendConfig(config: ClientConfig.Config): ClientConfigBackend.Config {
  return {
    ...config,
...
export function transToFrontConfig(config: ClientConfigBackend.Config): ClientConfig.Config {
  ...
  return {
    ...config,
    Remote: config.Remote.map(remote => {
      const [protocol, address] = remote.split("://");
      return {
        Protocol: protocol,
        Addr: address
      };
